# Google相册上传与批量下载 · Google Photos Uploader（Chrome 扩展）

Chrome Manifest V3 扩展：扫描当前网页图片 → 筛选并预览 → 批量上传到 Google Photos，或下载选中图片。ZIP 打包功能暂时停用，代码保留待后续恢复。支持任务进度、失败重试、断点续传和多种网页图片来源。
...
> 当前发行版为纯免费插件：不扣费、不限制上传/下载/ZIP 次数，也不依赖收费后台。收费后端与管理后台仅作为未来扩展代码保留，不参与当前运行。
...
- **下载**：下载选中图片到指定文件夹。ZIP 打包下载暂时停用（普通下载不受影响）。
...
- **免费使用**：上传和下载不需要产品账号、不需要支付、不连接收费后台；Google 账号仅在上传到 Google Photos 时授权。
...
5. **免费发行版**：无需配置产品账号、收费后端或支付。扩展的上传和下载不设次数配额；Google 账号仅在上传到 Google Photos 时授权。ZIP 打包当前暂时停用。
...
- 上传计入 Google 存储空间。
- 收费后端与管理后台代码仅用于未来商业版预留；当前发行版始终关闭额度校验，不读取产品账号、不扣费、不依赖后端。
