<template>
  <el-card shadow="never" class="panel-card">
    ...
    <el-table :data="orders" v-loading="loading" style="width:100%">
      ...
      <el-table-column label="金额" width="110"><template #default="{row}">¥{{ (row.amountCents/100).toFixed(2) }}</template></el-table-column>
      ...
      <el-table-column prop="providerTradeNo" label="渠道交易号" min-width="150" />
      <el-table-column label="支付时间" width="170"><template #default="{row}">{{ fmt(row.paidAt) }}</template></el-table-column>
...
async function replay(row: any) {
  try { await req('/admin/orders/' + row.orderNo + '/replay', { method: 'POST' }); ElMessage.success('已主动查单'); load(); }
